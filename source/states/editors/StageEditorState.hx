...
class StageEditorState extends MusicBeatState
{
	...
	private function _handleCamera(elapsed:Float):Void
	{
		...
		if (FlxG.keys.pressed.W || FlxG.keys.pressed.UP)    camGame.scroll.y -= speed;
		if (FlxG.keys.pressed.S || FlxG.keys.pressed.DOWN)  camGame.scroll.y += speed;
		if (FlxG.keys.pressed.A || FlxG.keys.pressed.LEFT)  camGame.scroll.x -= speed;
		if (FlxG.keys.pressed.D || FlxG.keys.pressed.RIGHT) camGame.scroll.x += speed;
	...
	private function _handleMouse():Void
	{
		if (FlxG.mouse.justPressed)
		{
			...
			for (i in 0...stageSprites.length)
			{
				...
				if (obj.sprite != null && FlxG.mouse.overlaps(obj.sprite, camGame))
				{
					...
					curObject      = i;
					isDragging     = true;
					dragOffsetX    = obj.sprite.x - FlxG.mouse.getWorldPosition(camGame).x;
					dragOffsetY    = obj.sprite.y - FlxG.mouse.getWorldPosition(camGame).y;
					break;
				}
			}
		}
		...
		{
			var wx:Float = FlxG.mouse.getWorldPosition(camGame).x;
			var wy:Float = FlxG.mouse.getWorldPosition(camGame).y;
	...
	private function _handleKeyboard():Void
	{
		...
		if (selectedSprite != null)
		{
			...
			{
				selectedSprite.x -= step;
				if (curObject >= 0) stageSprites[curObject].x = Std.int(selectedSprite.x);
			}
			...
			{
				selectedSprite.y -= step;
				if (curObject >= 0) stageSprites[curObject].y = Std.int(selectedSprite.y);
			}
			...
			if (FlxG.keys.justPressed.DELETE && curObject >= 0)
				_removeObject(curObject);
		}
	}
	...
	private function _handleTouch(elapsed:Float):Void
	{
		...
		if (touchPad.buttonUp2.justPressed)
		{
			showGrid            = !showGrid;
			gridOverlay.visible = showGrid;
		}

		if (touchPad.buttonDown2.justPressed)
		{
			camGame.scroll.set(0, 0);
			_setZoom(1.0);
		}

		if (touchPad.buttonA.justPressed && selectedSprite != null && curObject >= 0)
			_removeObject(curObject);
	...
	private function _buildUI():Void
	{
		...
		zoomTxt = new FlxText(FlxG.width - 120, 8, 110, 'Zoom: ${Math.round(camZoom * 100)}%', 14);
		zoomTxt.setFormat(Paths.font('vcr.ttf'), 14, FlxColor.WHITE, RIGHT);
		zoomTxt.cameras = [camUI];
		uiGroup.add(zoomTxt);

		#if !mobile
		helpTxt = new FlxText(10, FlxG.height - 90, FlxG.width - 20,
			'[WASD] Scroll  [Q/E] Zoom  [R] Reset  [G] Grid  [CTRL+Drag] Snap  [DEL] Delete  [F5] Save  [F6] Load  [ESC] Back',
			11);
		#else
		helpTxt = new FlxText(10, FlxG.height - 90, FlxG.width - 20,
			...
			11);
		#end
		helpTxt.setFormat(Paths.font('vcr.ttf'), 11, 0xFFCCCCCC, CENTER);
		helpTxt.cameras = [camUI];
		uiGroup.add(helpTxt);

		infoTxt = new FlxText(10, 44, 0, '', 13);
		infoTxt.setFormat(Paths.font('vcr.ttf'), 13, FlxColor.WHITE, LEFT);
	...
	private function _updateInfo():Void
	{
		...
		var info:String = 'Mouse: (${Std.int(wx)}, ${Std.int(wy)})  Objects: ${stageSprites.length}';

		if (selectedSprite != null && curObject >= 0)
		{
	...
	private function _drawGrid():Void
	{
		...
		var lineColor:Int = 0x33FFFFFF;

		var x:Int = 0;
		while (x < FlxG.width * 2)
		{
			for (py in 0...FlxG.height * 2)
				bmd.setPixel32(x, py, lineColor);
			x += GRID_SIZE;
		}
		...
		{
			for (px in 0...FlxG.width * 2)
				bmd.setPixel32(px, y, lineColor);
			y += GRID_SIZE;
		}
	...
	private function _toggleHelp():Void
	{
	...
	private function _loadStage():Void
	{
		try
		{
			...
			{
				CoolUtil.showPopUp('Stage file not found: $stageName', 'Error');
				return;
			}
			...
			CoolUtil.showPopUp('Stage "$stageName" loaded! (${stageSprites.length} objects)', 'Success');
		}
		catch (e:Dynamic)
		{
