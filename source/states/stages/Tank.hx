...
class Tank extends BaseStage
{
	...
	override function countdownTick(count:Countdown, num:Int):Void { if (num % 2 == 0) everyoneDance(); }
	override function beatHit():Void { everyoneDance(); }
	...
	function ughIntro():Void
	{
		...
		cutsceneHandler.timer(0.1, function():Void { wellWellWell.play(true); });
	...
	function gunsIntro():Void
	{
		...
		cutsceneHandler.timer(4, function():Void
		{
			...
			gf.animation.finishCallback = function(name:String):Void { gf.playAnim('sad', true); };
		});
	}
	...
	function stressIntro():Void
	{
		...
		cutsceneHandler.onStart = function():Void { cutsceneSnd.play(true); };
		...
		cutsceneHandler.timer(15.2, function():Void
		{
			...
			_onAnimComplete(pico, function():Void
			{
		...
		cutsceneHandler.timer(17.5, function():Void { zoomBack(); });

		cutsceneHandler.timer(19.5, function():Void { tankman.anim.play('lookWhoItIs', true); });

		cutsceneHandler.timer(20, function():Void { camFollow.setPosition(dad.x + 500, dad.y + 170); });
		...
		cutsceneHandler.timer(32.2, function():Void { zoomBack(); });
	}
	...
	private function _onAnimComplete(anim:FlxAnimate, cb:Void->Void):Void
	{
		var listener:Void->Void = null;
		listener = function():Void
		{
			anim.anim.onComplete.remove(listener);
			cb();
		};
		anim.anim.onComplete.add(listener);
	}

	private function _clearAnimComplete(anim:FlxAnimate):Void
	{
		anim.anim.onComplete.removeAll();
	}
}
