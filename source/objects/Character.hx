...
import support.codename.CodenameCharacter;
...
class Character extends FlxSprite
{
	...
	public function new(x:Float, y:Float, ?character:String = 'bf', ?isPlayer:Bool = false)
	{
		...
		{
			default:
				if (!_tryLoadCodename(curCharacter))
					_loadJson(curCharacter);
		}
	...
	private function _tryLoadCodename(character:String):Bool
	{
		var xmlPath:String = CodenameCharacter.findCharacterXml(character);
		if (xmlPath == null) return false;
		if (!CodenameCharacter.isCodenameXml(xmlPath)) return false;

		var data = CodenameCharacter.load(xmlPath);
		...
		try
		{
			...
			#if MODS_ALLOWED
			hasJson = FileSystem.exists(jsonPath);
			#else
			hasJson = Assets.exists(jsonPath);
			#end

			if (hasJson)
			{
				#if MODS_ALLOWED
				loadCharacterFile(Json.parse(File.getContent(jsonPath)));
				#else
				loadCharacterFile(Json.parse(Assets.getText(jsonPath)));
				#end
			}
			else
			{
				...
				healthColorArray = CodenameCharacter.hexToArray(data.color);
				vocalsFile       = '';
				jsonScale        = 1.0;
			}
		...
		{
			FlxG.log.warn('[Character] Failed to load Codename XML "$character": $e');
	...
	private function _loadJson(character:String):Void
	{
		...
		{
			FlxG.log.warn('[Character] Error loading "$character": $e');
		}
	}
	...
	override function update(elapsed:Float):Void
	{
		...
		if (!isPlayer && holdTimer >= Conductor.stepCrochet
			* (0.0011 #if FLX_PITCH / (FlxG.sound.music != null ? FlxG.sound.music.pitch : 1) #end)
			* singDuration)
		{
			dance();
			holdTimer = 0;
	...
	private function set_animPaused(value:Bool):Bool
	{
		...
		else { if (value) atlas.anim.pause(); else atlas.anim.resume(); }
		return value;
	}
	...
	public function recalculateDanceIdle():Void
	{
		...
		danceIdle = (animOffsets.exists('danceLeft' + idleSuffix)
			&& animOffsets.exists('danceRight' + idleSuffix));
	...
	{
		if (isAnimateAtlas) { copyAtlasValues(); atlas.draw(); return; }
		super.draw();
	}
